...
class CaseSpec:
    ...
    업계에서 흔한 일반 명칭만 쓴다 — 실제 단체의 인사규정 표기를 그대로
    옮기면 명부가 어느 회사 것인지 짚이는 단서가 된다.
    """
    wage_check: int = 0
    notes: tuple[str, ...] = ()
    """Input 시트에 남길 설명 줄."""
...
CASES: Final[tuple[CaseSpec, ...]] = (
    ...
    CaseSpec(
        ...
        groups=(
            ("정규직", 0.36), ("생산직", 0.20), ("관리직", 0.12),
            ("영업직", 0.10), ("계약직", 0.11), ("별정직", 0.04),
            ("임원", 0.07),
        ),
        notes=(
            "· 직군이 일곱이라 [직군 매핑] 탭에서 산출 묶음으로 묶어 주어야 합니다.",
    ...
    CaseSpec(
        key="특이케이스",
        title="시험명부3_특이케이스",
        summary="명부는 회사가 쓰듯 기본 항목만 깨끗하게 적혀 있습니다. 대신 "
                "[기초자료] 특이사항에 실무에서 실제로 만나는 사례들이 줄줄이 "
                "적혀 있습니다 — 무엇을 되물어 어떤 자료를 더 받고 어떻게 산출에 "
                "반영할지 계획하는 연습용입니다. 채무를 구하는 것이 목적이 아닙니다.",
        active=120, retired=12,
        groups=(("정규직", 0.78), ("계약직", 0.12), ("임원", 0.10)),
        notes=(
            "· 명부 자체는 오류 없이 깨끗합니다 — 특이사항이 문제지입니다.",
...
#: 특이케이스 명부의 문제지. (구분, 내용) — [기초자료] 특이사항 표에 그대로
#: 실린다. 명부 자체는 회사가 쓰듯 기본 항목만 깨끗하게 적고, 실습하는 사람이
#: 이 목록을 읽고 ① 더 받을 자료 ② 시스템 설정 ③ 산출 반영을 계획한다.
#: 채무를 구하는 것이 목적이 아니라 명부에 사례별 장치를 심지 않는다.
SPECIAL_NOTES: Final[tuple[tuple[str, str], ...]] = (
    ("사례 01", "장기급여를 재직자·퇴직자 모두 평가. 희망퇴직자에게 추가 지급이 "
              "있고 정년퇴직자에게도 지급한다."),
    ("사례 02", "연봉제 전환 — 전환 이전 근속은 누진제, 이후는 법정제. 중간정산 "
              "없이 지급률로 구간을 할당한다."),
    ("사례 03", "자녀 학자금 지급 규정이 있다 — 기타장기종업원급여로 평가할지 판단."),
    ("사례 04", "군경력 가산근속연수 — 법정제에서 지급률 근속에만 가산한다."),
    ("사례 05", "해외 주재원 임금이 엔화로 적혀 온다 — 환산 기준과 환율 가정."),
    ("사례 06", "입사 3년 이후부터 퇴직금 발생(1년당 0.5개월). 만근속 지급률이라 "
              "xx.dd 근속에서 '0.dd' 구간은 지급이 없다."),
    ("사례 07", "재직 중 사망하면 5,000만원을 추가로 지급한다."),
    ("사례 08", "동결(프로즌) DC 전환자 — 과거 근속분은 퇴직금(DB)으로 남고 이후분만 DC."),
    ("사례 09", "임원 퇴직소득 세법한도 동결(프로즌) — 2019년 이전 3배수·이후 2배수로 "
              "지급구간이 갈린다."),
    ("사례 10", "미사용 연차휴가의 소멸기한이 없다 — 쌓아 두었다가 퇴직 시 정산."),
    ("사례 11", "추계액을 DC형으로 운영 — 임원 DC 운용 적립금을 추계액에서 차감해 "
              "지급한다."),
    ("사례 12", "명예퇴직 제도 — 잔여 정년 개월수 × 기본급의 위로금."),
    ("사례 13", "보고서 4.1·4.2에 이자수익·기대수익 민감도를 요청받았다."),
    ("사례 14", "사망 추가지급 배수가 직원 1배·임원 1.5배(등기임원 2배)로 다르다."),
    ("사례 15", "24년 평가를 25년 평가에서 과거원가로 인식 — 인별 재측정 수정 산출."),
    ("사례 16", "장기포상 지급일 — 입사 1/1~9/30 은 그해 창립기념일(10/1), "
              "10/1~12/31 은 익년 창립기념일."),
    ("사례 17", "승진 시 유급휴가 5일, 근속 10년 도달 시 별도 기본급의 50% 지급."),
    ("사례 18", "2026-03-31 분기말 최초 평가 셋팅 — 사업연도말은 12월."),
    ("사례 19", "DC 전환 후 퇴직자 — 재직자·퇴직자명부에 사번이 중복된다. "
              "DC 전환 예정자·퇴직 예정자의 산출 방법도 정해야 한다."),
    ("사례 20", "전장직 등 특정 직군은 정년퇴직 시 기본급을 추가로 지급한다."),
    ("사례 21", "2년마다 건강검진비를 지급한다(원하는 기관에서 검진, 일부 지원)."),
    ("사례 22", "누진제 + 그룹 합산기산 — 임원은 근속을 반기 단위로 절상한다."),
    ("사례 23", "명부에 임의로 적은 항목이 많다 — 어떤 열을 믿고 무엇을 되물을지."),
    ("사례 24", "임금피크 — 만 57세부터 매년 10% 감액, 지급률은 피크 전 임금 기준."),
    ("사례 25", "계열사 전입자 — 그룹 입사일로 합산 근속을 인정하고 전입 인수액을 "
              "받았다."),
    ("사례 26", "기중에 사업부를 매각해 일부 인원이 전출됐다 — [추가명부] 대상."),
    ("사례 27", "정년을 60세에서 63세로 연장하는 노사합의 — 제도개정 과거근무원가."),
)
...
PRACTICE_CASES: Final[tuple[tuple[str, str], ...]] = (
    ...
    ("정년퇴직 시 기본급 추가지급",
     "정년으로 나가는 사람에게 기본급을 얹어 준다(특정직군 예우)."),
    ("휴직 차감",
...
def _case_report(
    ...
    if spec.flags.get("specials_only"):
        lines += [
            "",
            "[문제지 — 명부 기초자료의 특이사항 표와 같습니다]",
            "  명부 자체는 오류 없이 깨끗합니다. 사례마다 ① 더 받을 자료",
            "  ② 시스템 설정 ③ 산출 반영 방법을 적어 보세요.",
        ]
...
def _special_rows(spec: CaseSpec) -> dict[str, str]:
    """[기초자료] 특이사항 칸. 사례가 무엇을 담고 있는지 적어 둔다."""
    if spec.flags.get("specials_only"):
        # 특이케이스 명부 — 특이사항 표가 곧 문제지다.
        special = {"그 밖에": " ".join(note.lstrip("· ") for note in spec.notes)}
        special.update(SPECIAL_NOTES)
        return special
    special = {"그 밖에": " ".join(note.lstrip("· ") for note in spec.notes)}
