...
def _asset_numbers(
    ...
    return {
        "obligation": {
            "계열사에서 받음 (전입)": round(transfer_in),
            "합병·양수로 받음": 0,
            "퇴직자 지급액": round(paid["benefits_paid"]),
            "중간정산 지급액": round(settlement),
            "DC 전환 지급액": round(paid["dc_converted"]),
            "위로금·명예퇴직금": round(paid["other_paid"]),
            "계열사로 보냄 (전출)": round(paid["transfer_out"]),
            "사업 매각·분할로 보냄": round(paid["disposal"]),
        },
        "opening": (round(opening - national_pension), round(national_pension)),
        "asset": {
            "회사 납입 부담금": (round(contributions), 0),
            "운용수익 (이자 등)": (round(actual_return), 0),
            "계열사에서 받음 (전입)": (0, 0),
            "합병·양수로 받음": (0, 0),
            "퇴직자 지급액": (round(fund_paid), 0),
            "중간정산 지급액": (0, 0),
            "DC 전환 지급액": (0, 0),
            "계열사로 보냄 (전출)": (0, 0),
            "사업 매각·분할로 보냄": (0, 0),
            "수수료 (운용관리)": (round(management_fee), 0),
            "수수료 (자산관리)": (round(custody_fee), 0),
        },
