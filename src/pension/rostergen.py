...
``자료불량``
    ...
    퇴직, 명예퇴직, 사망 유족지원금 같은 것들이며 비고란에 무엇인지 적어 둔다.
...
CASES: Final[tuple[CaseSpec, ...]] = (
    ...
    CaseSpec(
        ...
        groups=(
            ("정사원", 0.46), ("생산직", 0.20), ("촉탁사원", 0.13),
            ("계약직(기간제)", 0.09), ("파견", 0.04), ("별정직", 0.03),
            ("임원（주재원）", 0.03), ("등기임원", 0.02),
        ),
        notes=(
    ...
    CaseSpec(
        ...
        notes=(
            ...
            "  명예퇴직 예정자 · 정년 시 기본급 추가지급 · 가산근속 · 사망 유족지원금 ·",
...
def _add_practice_cases(
    ...
    if len(retirees) >= 2:
        ...
        dead["note"] = "재직 중 사망 — 유족지원금 5,000만원 별도"
