...
지급률 규정은 사유별로 ``지급률``·``지급률 사망``·``지급률 정년`` 같은
...
#   '휴가 10일', '현물 포상 + 기념품', '평균임금의 500%', '100만원'
...
class SalaryScale:
    ...
    ``일반사항`` 시트가 "승진·승급으로 인한 임금인상 제외" 한 Base-up 을 따로
    받는 것과 같은 구조다.
    """
...
class CauseBenefit:
    ...
    자료요청서 6번에도 세 줄이 따로 있다.
        ...
        사망시 퇴직금 지급률       동일, 정액 가산 있음
        정년퇴직시 퇴직금 지급률   임원은 배수 별도
    ...
    """정액 가산액(원). 근속과 무관하게 얹는 금액.

    **전원에게 같은 금액** 이다. 사람마다 다른 위로금은 이 칸으로 담을 수
    없으므로 :attr:`roster_extra_multiple` 을 쓴다.
    """
    roster_extra_multiple: float = 0.0
    """명부 [추가지급 기본급] 칸에 곱해 얹을 배수.

    사망 위로금·유족 일시금처럼 **사람마다 금액이 다른** 가산을 담는다. 그런
    금액은 명부에 사람별로 적혀 오는데, 회사마다 그 칸의 뜻이 달라(위로금인
    곳, 한도인 곳, 누적 배수인 곳) 엔진이 스스로 얹으면 뒤의 두 경우에서
    채무가 통째로 틀린다. 그래서 **규정이 쓰겠다고 말할 때만** 얹는다.

    ``1`` 이면 명부에 적힌 금액 그대로, ``0.5`` 면 그 절반이다. 0(비움)이면
    쓰지 않는다 — 지금까지처럼 명부 칸은 참고로만 남는다.
    """
...
def _read_exit_causes(wb) -> tuple[CauseBenefits, list[str]]:
    ...
    for row, _ in _rows(ws):
        ...
        entry = CauseBenefit(
            ...
            attribution=basis,
            # 8번째 열은 나중에 붙였다. 없는 파일은 빈 칸으로 읽히므로 예전
            # 기초율도 그대로 돌아간다.
            roster_extra_multiple=_as_number(ws.cell(row, 8).value) or 0.0,
        )
        if entry.is_empty():
...
def write_template(path: str | Path, *, job_groups: Iterable[str] = (),
    ...
    make(
        ...
        "사람마다 금액이 다른 위로금은 이쪽입니다 — 1 이면 적힌 금액 그대로, "
        "0.5 면 절반. 비우면 명부 칸을 쓰지 않습니다"
        "\n· 예) 전원 같은 금액이면  정규직 | 사망 | (비움) | (비움) | 50000000 | 1 | (비움) | (비움)"
        "\n· 예) 사람마다 다르면    정규직 | 사망 | (비움) | (비움) | (비움) | 1 | (비움) | 1",
        [],
    )
