...
VBA 는 임직원구분·성별·퇴직사유를 여러 표기로 받아 표준값으로 바꾼다. 여기서는
...
# VBA: rvar = "Y"/"y"/"임원"/"임"/2 이면 임원, 그 외 전부 직원.
_EXECUTIVE_TOKENS: Final[frozenset[str]] = frozenset({"y", "임원", "임", "2"})

# VBA: rvar = "녀"/"여"/"여자"/"여성"/2/4/6/8 이면 여자, 그 외 전부 남자.
...
def normalize_gender(value: object) -> Gender:
    """성별 정규화. 판정되지 않는 값은 VBA 와 같이 '남자' 로 본다."""
    token = text(value).lower()
...
def normalize_retirement_reason(value: object) -> RetirementReason | None:
    ...
    VBA 는 셀 앞 두 글자만 잘라(``Left(cell, 2)``) 판정한다. 그래서 '중도퇴직'은
    '중도', '사업처분/분할'은 '사업' 으로 매칭된다. 같은 규칙을 유지하되
    '계약만료'(→ 중도퇴직) 처럼 시트 수식에만 있던 대응도 함께 처리한다.

    ``'임금'``(임금피크제도에 따른 중간정산)은 VBA 를 따라 4(정년퇴직)로 본다.
...
def is_ambiguous_reason(value: object) -> bool:
    """VBA 규칙과 시트 수식의 분류가 어긋나는 지급사유인지.

    '임금피크제도에 따른 중간정산'은 VBA ``Case "4", "정년", "임금"`` 에 따라
