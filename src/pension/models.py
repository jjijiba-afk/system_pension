...
class ActiveMember:
    ...
    :attr:`mixed_plan_start_date` 가 있으면 **그 날 이후 근속에만** 걸린다.
    """
    mixed_plan_start_date: _dt.date | None = None
    """혼합형에 가입한 날. 비어 있으면 입사 때부터 혼합형이었던 것으로 본다.

    이 날짜가 없으면 채무가 틀린다. 10년 일한 사람이 작년에 `DB 50 / DC 50`
    혼합형으로 바꿨다면, 그 전 10년치는 **전액 DB** 로 남아 있다. 비율만 받아
    전 근속에 걸면 그 10년치의 절반이 채무에서 통째로 사라진다.

    회사가 혼합형을 도입하는 것은 보통 제도 개편 때다 — 그날 이전 근속을 DC 로
    넘기려면 근로자 동의와 실제 이전이 필요하고, 그러면 중간정산일로 온다.
    """
    ...
    :attr:`contract_end_date` 가 있으면 그것으로 다시 계산해 덮는다.
    """
    contract_end_date: _dt.date | None = None
    """근로계약 종료일. 있으면 :attr:`remaining_contract_years` 를 여기서 낸다.

    연수로 받으면 두 가지가 어긋난다. 첫째, **기준일이 바뀌는 순간 틀린다** —
    전기 명부를 그대로 다시 쓰면 남은 기간이 한 해 줄었는데도 그대로 2년이다.
    둘째, 적는 사람이 반올림한다 — 1년 5개월 남은 사람이 흔히 2년으로 온다.
    날짜는 둘 다 겪지 않는다. 회사 인사자료에도 날짜로 들어 있다.
    """
...
class RetiredMember:
    ...
    """퇴직급여 총지급금액 — **채무 전표처리 기준**.

    퇴직으로 채무가 줄어든 금액이다. 증감표의 급여지급액이 이 값이며, 산출도
    이것을 쓴다.
    """
    cash_payment: float = 0.0
    """**실지급 기준** 지급액. 실제로 회사 계좌에서 나간 금액이다.

    두 값이 갈리는 것이 증감표가 안 맞는 흔한 원인이다. 12월에 퇴직한 사람의
    돈이 1월에 나가면 전표는 당기, 현금은 차기다. 원천징수를 뗀 뒤 금액으로
    적어 오는 회사도 있다.

    어느 쪽이 맞고 틀린 것이 아니라 **기준이 다른 두 숫자** 다. 한 칸으로
    받으면 어느 기준으로 적었는지 알 수가 없어, 증감표가 안 맞을 때 원인을
    짚지 못한다. 비어 있으면 전표 기준과 같은 것으로 본다.
    """
