...
날짜 칸은 여럿이다 — 생년월일·입사일·중간정산일·전입일·추가지급 기준일·
퇴사일·사외자산 지급일. 명부마다 서식이 제각각이라 판정이 길어지는데, 칸마다
같은 판정을 따로 두면 한쪽만 고쳐져 같은 값이 칸에 따라 다르게 읽힌다.
그래서 여기 한 곳에 모은다.
...
def parse_roster_date(value: object, base_year: int) -> _dt.date | None:
    ...
    길이 8의 ``yy.mm.d일`` 은 주의가 필요하다. 일자는 **7번째** 자리에 있다 —
    6번째는 구분자 ``.`` 라, 거기서 읽으면 어떤 값을 넣어도 해석에 실패한다.
    """
    if value is None:
        return None
...
def _parse_text(s: str, base_year: int, raw: str) -> _dt.date:
    ...
    if leng == 8:
        ...
        if _is_digit(s, 5):
            ...
            # yy.mm.d일 — 일자는 7번째다. 6번째를 읽으면 구분자를 읽게 된다.
            return _ymd(year, _num(s, 4, 2, raw), _num(s, 7, 1, raw), raw)
        # yy.m.dd일
        return _ymd(year, _num(s, 4, 1, raw), _num(s, 6, 2, raw), raw)
