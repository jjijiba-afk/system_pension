...
* 그대로 두면 산출이 틀어지는 치명적 오류(사번 중복, 직군 미매칭, 연령 범위,
  평균임금 하한, 입사일 > 기준일 등).
* 눈으로 훑어서는 잘 보이지 않는 것(필수값 누락, 날짜 선후관계,
  사외자산 지급액 > 총지급액 등).

첫 오류에서 멈추지 않고 전부 모아 한 번에 돌려준다. 담당자가 명부를 한 번만
손보면 되도록 하는 것이 목적이다.
"""
...
def _check_duplicate_ids(members, sheet: str, log: IssueLog, code: str) -> None:
    ...
    첫 중복에서 멈추지 않는다. 사번별로 묶어 한 번에 훑고, 중복된 사번을 모두
    보고한다. 사번이 비어 있는 행은 중복 검사에서 제외한다(뒤에서 자동 생성하기
    때문).
    """
