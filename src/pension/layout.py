...
ACTIVE_HEADER_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    # 비율만 받으면 채무가 틀린다 — 혼합형 가입 전 근속은 전액 DB 다.
    "mixed_plan_start_date": ("혼합형가입일", "혼합형기준일", "혼합형가입기준일",
                              "혼합형전환일", "제도전환일", "DC전환일", "dc전환일"),
...
RETIRED_HEADER_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    "total_payment": ("퇴직급여총지급금액", "총지급금액", "퇴직급여총지급액",
                      "채무전표처리기준지급액", "전표처리기준지급액", "발생기준지급액"),
    # 전표와 현금은 시점이 갈린다. 한 칸으로 받으면 어느 기준인지 알 수 없어
    # 증감표가 안 맞을 때 원인을 짚지 못한다.
    "cash_payment": ("실지급기준지급액", "실지급액", "실지급금액", "실제지급액",
                     "현금지급액", "실지급기준"),
