...
``명부_기본.xlsx``
    실제 평가 사례의 명부(재직 275명·퇴직 247명). 예시 두 줄로는 볼 수 없는
    실제 형태가 그대로 들어 있다.
...
def write_standard_assumptions(
    ...
    *,
    ...
    make(
        ...
        [[0, *[1.0] * n]],
        "· 30일 평균임금 대비 지급배수입니다. 위 한 줄(근속 0년 → 연 1.0배)은 법정 퇴직금과 같습니다.\n"
        "· 누진제라면 구간별 연 배수를 줄마다 적고 '지급률규정' 방식을 '누진' 으로 두세요.\n"
        "· 첫 줄의 근속연수를 0 으로 두는 것을 잊지 마세요. 1 로 적으면 첫 1년이 빠집니다.",
        warn="[확인] 회사 퇴직금 규정이 법정(1년당 30일분)이 아니면 반드시 바꾸세요.",
    )
    make(
        BENEFIT_RULE_SHEET, ["규정명", "방식", "수식", "설명"],
        [[g, PROGRESSIVE, "", "법정 퇴직금 (근속 1년당 30일분)"] for g in groups],
        "· 방식: 누적(표 값이 누적 배수) / 누진(표 값이 구간별 연 배수) / 수식\n"
        "· 수식 변수: t=근속연수, x=연령, N=정년연령, S=30일 평균임금, 제도, 직군, 임직원, 배수\n"
        "· 되도록 누진(표)을 쓰세요. 수식은 표로 담기 어려운 규정에만 씁니다.",
    )
    make(
    ...
    make(
        ...
        "· 지급유형: 휴가 / 평균임금 / 현물 / 현금\n"
        "· 현물(순금 등)만 상승률을 따로 받습니다. 시세는 평가시점으로 환산해 넣고 근거를 남기세요.",
    )
