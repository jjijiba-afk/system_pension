...
VBA 는 이 값들을 매크로 실행 때마다 ``Cells(3, 3).Value`` 처럼 직접 읽는다.
...
#: VBA 가 훑는 직군 행 수 상한(``jkcnt`` 배열이 1 To 25 로 선언되어 있다).
_RULE_MAX_ROWS = 25
...
class CalculationConfig:
    ...
    def find_job_group(
        ...
        VBA 는 직군 완전일치 하나만 보았다. 임원의 직군이 '정규직' 으로 적혀
        오는 명부가 있어 임원을 갈라낼 수 없었다.

        원문까지 보는 이유는 정규화가 `임원`/`직원` 둘로만 줄이기 때문이다.
        같은 `과장` 이라도 `정사원` 과 `촉탁사원` 은 계약 형태가 달라 퇴직률과
...
def read_config(workbook, sheet_name: str = INPUT_SHEET) -> CalculationConfig:
    ...
    for offset in range(_RULE_MAX_ROWS):
        ...
        # VBA 는 CountA(C12:C26) 으로 행 수를 세지만, 중간에 빈 행이 있으면
