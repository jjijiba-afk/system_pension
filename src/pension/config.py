...
산출 기준은 한 번 읽어 :class:`CalculationConfig` 로 고정한 뒤 파이프라인
전체에 넘긴다. 필요할 때마다 시트를 다시 읽으면, 중간에 무엇이 바뀌었을 때
앞뒤가 다른 기준으로 산출된다.
"""
...
class CalculationConfig:
    ...
    def find_job_group(
        ...
        직군 완전일치 하나만 보면 임원을 갈라낼 수 없다. 임원의 직군이
        '정규직' 으로 적혀 오는 명부가 있기 때문이다.
