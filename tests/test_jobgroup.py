...
`임직원구분` 에만 `정규사원`/`촉탁사원`/`임원(별정)` 이 적힌 경우가 있었다.
"""
    ...
    def _config(self) -> CalculationConfig:
        return CalculationConfig(
            ...
            job_group_rules=[
                ...
                JobGroupRule("과장", "정규직", 60, 60, 2, employee_type_filter="정규사원"),
                JobGroupRule("과장", "임원", 65, 65, 2, employee_type_filter="임원(별정)"),
                JobGroupRule("사원", "정규직", 60, 60, 2, employee_type_filter="정규사원"),
            ]
        )

    def test_raw_employee_type_splits_the_same_job(self) -> None:
        """정규화하면 `정규사원`·`촉탁사원` 이 똑같이 '직원' 이라 구분이 사라진다."""
        config = self._config()
        assert config.find_job_group("과장", "직원", "정규사원")[1].mapped_name == "정규직"
        assert config.find_job_group("과장", "직원", "촉탁사원")[1].mapped_name == "계약직"
        assert config.find_job_group("과장", "임원", "임원(별정)")[1].mapped_name == "임원"
...
class TestScanRoster:
    ...
    def test_counts_and_ordering(self, tmp_path) -> None:
        ...
        assert by_key[("과장", "정규사원")].active == 2
        assert by_key[("과장", "정규사원")].retired == 1
        assert by_key[("과장", "촉탁사원")].headcount == 1
        assert by_key[("대표이사", "임원(별정)")].normalized_type == "임원"
        # 인원 많은 순
        assert found[0].key == ("과장", "정규사원")
...
class TestPipelineUsesPayoutSheet:
    ...
    def test_payout_sheet_overrides_the_roster(self, tmp_path) -> None:
        ...
        assert by_kind == {"정규사원": "정규직", "촉탁사원": "계약직"}
